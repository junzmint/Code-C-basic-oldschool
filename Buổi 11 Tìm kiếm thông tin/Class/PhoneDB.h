...
int SearchPhoneMoveToFront(char model[20]){
    node *p;
    for (p = root; p != NULL && strcmp(p->element.model,model) != 0; p = p->next);
    DeleteAtCur();
    InsertHead(p->element);
    return 1;
}

int SearchPhoneTranpose(char model[20]){
    node *p;
    for (p = root; p != NULL && strcmp(p->element.model,model) != 0; p = p->next);
    DeleteAtCur();
    InsertHead(p->element);
