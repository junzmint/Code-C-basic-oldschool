...
void f_print2DUtil(FILE *fp,treeType root, int space)
{
    // Base case
    if (root == NULL)
        return;
    ...
    // Process right child first
    f_print2DUtil(fp,root->right, space);
    ...
    for (int i = 10; i < space; i++)
        fprintf(fp," ");
    fprintf(fp,"%s", root->element);

    // Process left child
    f_print2DUtil(fp,root->left, space);
}

// Wrapper over print2DUtil()
void f_drawTree(FILE *fp,treeType root)
{
    // Pass initial space count as 0
    f_print2DUtil(fp,root, 0);
}
