    ...
    for (int i = 0; i < File_length; i++)
    {
        ...
        if (strcmp(search, tmp) == SUCCESS)
        {
            ...
            if (choice == 1)
            {
                ...
                strcpy(search, tmp_node->element.name);
            }
            if (searchType == 1)
            {
                node *tmp = *root;
                *cur = tmp;
                while (tmp != NULL)
                {
                    if (strcmp(tmp->element.name, search) == 0 || strcmp(tmp->element.mem_space, search) == 0 || strcmp(tmp->element.screen_size, search) == 0 || strcmp(tmp->element.price, search) == 0)
                    {
                        if (*cur != *root)
                        {
                            moveToFrontver2(root, tail, cur);
                        }
                    }
                    tmp = tmp->next;
                    *cur = tmp;
...
int Option11(FILE *fptr, int File_length, doublelist *root)
{
    ...
    fseek(fptr, 0, SEEK_SET);
    storeLinkedList(file_out, File_length, root);
    fclose(file_out);
    getchar();
    printf("Press Enter to continue!");
}